...
#include "MultiCamCharacter.generated.h"


// TILAN: - Camera Mode Enumeration
...
namespace ECharacterCameraMode
{
	enum    Type
	{
		...
		ThirdPersonReseting     UMETA(DisplayName="Third Person Smooth Follow"),
...
static inline FString GetNameForCameraMode(const ECharacterCameraMode::Type CameraMode)
{
	switch(CameraMode)
	{
		...
		case ECharacterCameraMode::FirstPerson:
			...
			break;
		case ECharacterCameraMode::ThirdPersonReseting:
...
// TILAN: - AMultiCamCharacter

UCLASS(config=Game)
class AMultiCamCharacter : public ACharacter
{
	GENERATED_BODY()
...
private:
	void ResettingTick(float DeltaSeconds);
	void ThirdPersonResetingTick(float DeltaSeconds);
