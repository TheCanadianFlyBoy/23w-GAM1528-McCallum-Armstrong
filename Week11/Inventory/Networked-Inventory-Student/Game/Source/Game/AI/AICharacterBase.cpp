...
void AAICharacterBase::OnPawnSeen(APawn* SeenPawn)
{
	...
	//IF Cast<ACharacterBase>(SeenPawn)->GetHealth() NOT EQUAL TO nullptr AND Cast<ACharacterBase>(SeenPawn)->GetHealth()->IsDead()
   	if (Cast<ACharacterBase>(SeenPawn)->GetHealth() != nullptr && Cast<ACharacterBase>(SeenPawn)->GetHealth()->IsDead())
    {
...
void AAICharacterBase::Tick(float DeltaTime)
{
	...
	if (CurrentPatrolPoint)
	{
		...
		bool nearPatrol = GetActorLocation().Equals(CurrentPatrolPoint->GetActorLocation(), 130);
	...
	if (TargetActor)
	{
		...
		SetActorRotation(FMath::Lerp(GetActorRotation(), Dir.Rotation(), 1));
	}
	//ENDIF
}
