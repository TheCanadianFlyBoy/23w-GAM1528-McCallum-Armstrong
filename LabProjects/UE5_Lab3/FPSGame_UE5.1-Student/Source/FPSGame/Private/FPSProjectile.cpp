...
void AFPSProjectile::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if ((OtherActor != NULL) && (OtherActor != this) && (OtherComp != NULL) && OtherComp->IsSimulatingPhysics())
	{
		...
		if (!Cast<AFPSCharacterUE5Character>(OtherActor) && !Cast<AFPSBombActor>(OtherActor))
		{
...
{
	FVector Scale = OtherComp->GetComponentScale() * 0.8f;
	...
	else
	{
		...
		SetRandomColour(OtherComp, FLinearColor::MakeRandomColor());
	}
}

void AFPSProjectile::SetRandomColour(UPrimitiveComponent* OtherComp, FLinearColor color)
{
